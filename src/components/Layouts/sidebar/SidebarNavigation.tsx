...
import { getDynamicNavData, INITIAL_NAV_DATA } from "./data";
...
export function MobileMenu({
  ...
  useEffect(() => {
    ...
    if (!propRole) {
      ...
      console.log("Mobile Menu - Retrieved role:", userRole);
      setInternalRole(userRole);
    }

    if (propPermissions.length === 0) {
      const userPermissions = getDecryptedItem("permissions") || [];
      console.log("Mobile Menu - Retrieved permissions:", userPermissions);
    ...
    <div className="fixed inset-0 z-50 bg-white dark:bg-gray-dark">
      ...
      <div className="relative h-full max-h-[80vh] overflow-y-auto rounded-t-2xl bg-white p-6 shadow-lg dark:bg-gray-dark">
        ...
        <div className="space-y-6">
          {filteredNavData.length > 0 && filteredNavData[0].items.length > 0 ? (
            ...
            <div className="text-center text-gray-500 dark:text-gray-400">
              <p>No menu items available for your role and permissions.</p>
              <p className="mt-2 text-sm">
                Role: {role || "Not set"} | Permissions: {permissions.length}
              </p>
            </div>
          )}
        </div>
